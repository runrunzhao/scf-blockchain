...
public class ContractDAO {
    ...
    public List<Contract> searchContracts(String contractId, String enterpriseName,
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            while (rs.next()) {
                ...
                // 转换为 java.util.Date
                if (signingDate != null)
                    contract.setSignDate(new java.util.Date(signingDate.getTime()));
                if (effectiveDate != null)
                    contract.setEffectiveDate(new java.util.Date(effectiveDate.getTime()));
                if (invalidDate != null)
    ...
    public void updateContract(Contract contract) throws SQLException {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            int rowsAffected = stmt.executeUpdate();
            if (rowsAffected == 0) {
                throw new SQLException("Failed to update contract, no rows affected. Contract ID may not exist: "
