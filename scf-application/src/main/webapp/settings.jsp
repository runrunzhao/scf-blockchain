<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ page import="java.sql.*" %>
        <%@ page import="com.supplychainfinance.util.DBUtil" %>
            <% // Get current username from session String username=(String) session.getAttribute("username"); String
                userEmail="" ; String enterpriseId="" ; String walletAddr="" ; // Load fresh data from database if
                (username !=null) { try (Connection conn=DBUtil.getConnection()) { String
                sql="SELECT email, enterprise_id, walletAddr FROM users WHERE username = ?" ; try (PreparedStatement
                stmt=conn.prepareStatement(sql)) { stmt.setString(1, username); try (ResultSet rs=stmt.executeQuery()) {
                if (rs.next()) { userEmail=rs.getString("email"); enterpriseId=rs.getString("enterprise_id") !=null ?
                rs.getString("enterprise_id") : "" ; walletAddr=rs.getString("walletAddr") !=null ?
                rs.getString("walletAddr") : "" ; } } } } catch (Exception e) { e.printStackTrace();
                    ...
                    <div class="header">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="user-status">
                                <% if(session !=null && session.getAttribute("username") !=null) { // Remove the
                                    duplicate declaration - username is already defined %>
                                    <div class="dropdown">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control" id="walletAddr" name="walletAddr"
                                                placeholder="Enter your ETH address" value="<%= session.getAttribute("
                                                walletAddr") !=null ? session.getAttribute("walletAddr") : "" %>">
