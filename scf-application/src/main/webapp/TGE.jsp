        ...
        <div class="container">
                ...
                <div class="card-body">
                    <form id="mintForm">
                        <div class="form-group">
                            ...
                            <label for="contractAddressDisplay"><strong>SC Address:</strong></label>
            ...
            <script>
                ...
                function loadLatestToken() {
                    // Show loading status
                    showStatus("Loading latest token information...");

                    // Fetch the latest contract from server without filtering by wallet address
                    fetch('searchToken')
                        .then(response => {
                            if (!response.ok) {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                // Make sure amount field is empty
                                ...
                                if (token.genContractAddr) {
                                    ...
                                    // If web3 is already initialized, create contract instance
                                    if (web3 && token.genContractAddr) {
                                        try {
                                            contract = new web3.eth.Contract(customTokenABI, token.genContractAddr);
                                            console.log("Contract instance created automatically from loaded data");
                                        } catch (contractError) {
                                ...
                                showStatus("Latest token loaded successfully");
                            } else {
                                // Show the message that allows manual creation
                                showStatus("No existing token found. You can create a new one.");
                            }
                        })
                        .catch(error => {
                            console.error('Error loading latest token:', error);
                            showStatus("Failed to load latest token: " + error.message, true);
                        });
                }
                ...
                async function mintToken(recipientAddress, amount) {
                    ...
                    // Check if contract is not initialized but we have an address
                    if (!contract && document.getElementById('contractAddressDisplay').value) {
                        try {
                            // Initialize contract using the displayed address
                            ...
                            console.log("Contract initialized from displayed address");
                        } catch (error) {
                            console.error("Failed to initialize contract:", error);
                            showStatus("Failed to initialize contract: " + error.message, true);
                            return false;
                        }
                    }

                    if (!contract) {
                        showStatus("Contract not deployed. Please click 'Sign Contract' first to deploy the token contract.", true);
                        return false;
                    }

                    try {
                        // Check expiration date first to provide clearer error message
                        try {
                            ...
                            console.log("Could not check expiration date:", expError);
                        }

                        // 将数量转换为正确的格式 (假设使用 18 位小数)
                        ...
                        showStatus("Minting tokens, please confirm transaction in your wallet...");
                        ...
                        const tx = await contract.methods.mint(recipientAddress, amountInWei).send({
                            from: userAddress,
                            gas: 3000000, // 固定较大的gas数量
                            gasPrice: currentGasPrice // 使用当前gas价格
                        });

                        showStatus("Token minted successfully! Transaction hash: " + tx.transactionHash);
                        return true;
                    } catch (error) {
                        ...
                        showStatus("Error minting token: " + error.message, true);
                        return false;
                    }
                }
            </script>
