    ...
    <body>
        <div class="container">
            <div class="card wallet-section">
                ...
                <div class="card-body">
                    <div id="walletStatus" class="mb-2">Connected</div>
            ...
            <script>
                ...
                async function mintToken(recipientAddress, amount) {
                    ...
                    try {
                        ...
                        // Check gas estimate before sending transaction
                        let gasEstimate;
                        try {
                            gasEstimate = await contract.methods.mint(recipientAddress, amountInWei).estimateGas({
                                from: userAddress
                            });
                            ...
                            if (gasError.message.includes("execution reverted")) {
                                ...
                                showStatus("Gas estimation failed: " + gasError.message, true);
                            }
                            return false;
                        }
                        ...
                        const tx = await contract.methods.mint(recipientAddress, amountInWei).send({
                            from: userAddress,
                            gas: Math.min(Math.floor(gasEstimate * 3), 1500000), // Increased from 1.5x to 3x with higher cap
                            maxFeePerGas: null, // Let MetaMask determine appropriate values
                        ...
                        if (error.receipt) {
                            ...
                            errorMsg = "Transaction reverted. Status: " + error.receipt.status;
                        }

                        // 检查是否包含常见错误
                        if (errorMsg.includes("gas")) {
                            errorMsg = "Transaction failed due to gas issues. Try increasing gas limit.";
                        } else if (errorMsg.includes("rejected")) {
                            errorMsg = "Transaction was rejected in your wallet.";
                        } else if (errorMsg.includes("expired") || errorMsg.includes("Token has expired")) {
                            errorMsg = "Token has expired according to the contract's expiration date.";
                        } else if (errorMsg.includes("Internal JSON-RPC error")) {
                            // Try to extract the real error message from the stack trace
                            try {
                                const stackData = JSON.parse(error.stack);
                                if (stackData.data && stackData.data.message) {
                                    errorMsg = "Contract error: " + stackData.data.message;
                                } else {
                                    errorMsg = "Transaction reverted by the blockchain. Check that the token hasn't expired.";
                                }
                            } catch (e) {
                                errorMsg = "Transaction reverted by the blockchain. Check that the token hasn't expired.";
                            }
                        ...
                        showStatus("Error minting token: " + errorMsg, true);
                        return false;
                    }
                }
